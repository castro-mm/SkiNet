...
export class AddressPipe implements PipeTransform {
    ...
    transform(value: ConfirmationToken['shipping'] | ShippingAddress | undefined, ...args: unknown[]): unknown {
        if (value && 'address' in value && value.name) {
            const {line1, line2, city, state, country, postal_code} = (value as ConfirmationToken['shipping'])?.address!; // value.address;
